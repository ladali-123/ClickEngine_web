
const GoogleAdsManagement = () => {
  return (
    <div className="bg-white">
      {/* Hero Section - Two columns with image */}
      <section className="max-w-7xl mx-auto px-6 lg:px-12 py-12 lg:py-14">
        <div className="grid md:grid-cols-2 gap-10 items-center">
          {/* Left side - Text */}
          <div>
            <span className="inline-block px-4 py-1.5 rounded-full bg-blue-50 text-blue-600 text-sm font-semibold tracking-wide mt-12">
              Google Ads Management
            </span>

            <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold text-slate-900 mt-5 leading-tight">
              Pay Less. Get More Clicks.
              <span className="text-blue-600 block mt-1">
                Maximize Your ROI.
              </span>
            </h1>

            <p className="text-gray-600 text-base md:text-lg mt-5 leading-relaxed">
              Our certified Google Ads experts create high-converting campaigns
              that drive targeted traffic, reduce cost-per-click, and deliver
              measurable results for your business.
            </p>

            <button className="mt-6 bg-blue-600 hover:bg-blue-700 text-white px-7 py-2.5 rounded-lg font-semibold transition shadow-sm hover:shadow-md">
              Contact Us →
            </button>
          </div>

          {/* Right side - Reliable image */}
          <div className="flex justify-center">
            <img
              src="https://i.pinimg.com/1200x/b3/02/77/b30277ab18dc809d4c7f59b3c21ee102.jpg"
              alt="Google Ads Dashboard Illustration"
              className="w-full max-w-md rounded-2xl shadow-lg object-cover"
            />
          </div>
        </div>
      </section>

      {/* Why Google Ads - Compact and clean */}
      <section className="max-w-7xl mx-auto px-6 lg:px-12 py-10 lg:py-12 border-t border-gray-100">
        <div className="max-w-4xl">
          <h2 className="text-2xl md:text-3xl font-bold text-slate-900 mb-4">
            Why Google Ads Works
          </h2>

          <div className="space-y-3 text-gray-600 leading-relaxed">
            <p>
              Google Ads puts your business in front of customers who are actively
              searching for your products or services. Unlike traditional advertising,
              you only pay when someone clicks your ad – making it one of the most
              cost-effective marketing channels.
            </p>

            <p>
              We manage every aspect of your campaigns – from keyword research and ad
              copy to bid optimization and A/B testing – ensuring maximum ROI and
              continuous improvement.
            </p>
          </div>
        </div>
      </section>

      {/* Services - Two column layout with cards feel */}
      <section className="max-w-7xl mx-auto px-6 lg:px-12 py-10 lg:py-12">
        <div className="grid lg:grid-cols-2 gap-10 items-start">
          <div>
            <h2 className="text-2xl md:text-3xl font-bold text-slate-900 mb-4">
              Our Google Ads Solutions
            </h2>
            <p className="text-gray-600 leading-relaxed">
              We provide end-to-end Google Ads management tailored to your business
              goals – from Search and Display to Shopping and YouTube campaigns.
            </p>
          </div>

          <div className="bg-gray-50 rounded-xl p-6 shadow-sm">
            <ul className="grid sm:grid-cols-2 gap-3">
              {[
                "Keyword Research & Bidding",
                "Ad Copy & Extensions",
                "Landing Page Optimization",
                "Remarketing Campaigns",
                "Shopping & YouTube Ads",
                "Monthly Performance Reports",
              ].map((item, index) => (
                <li
                  key={index}
                  className="flex items-center gap-2.5 text-gray-700 font-medium"
                >
                  <span className="w-1.5 h-1.5 bg-blue-500 rounded-full"></span>
                  {item}
                </li>
              ))}
            </ul>
          </div>
        </div>
      </section>

      {/* Stats - Card style with less height and better visual */}
      <section className="max-w-7xl mx-auto px-6 lg:px-12 py-10 lg:py-12 border-t border-gray-100">
        <div className="grid md:grid-cols-3 gap-5">
          <div className="bg-white border border-gray-100 rounded-xl p-5 text-center shadow-sm hover:shadow-md transition">
            <h3 className="text-3xl md:text-4xl font-extrabold text-blue-600">42%</h3>
            <p className="text-gray-500 text-sm mt-1">Lower Cost-Per-Click</p>
          </div>

          <div className="bg-white border border-gray-100 rounded-xl p-5 text-center shadow-sm hover:shadow-md transition">
            <h3 className="text-3xl md:text-4xl font-extrabold text-blue-600">3.2x</h3>
            <p className="text-gray-500 text-sm mt-1">Average ROAS</p>
          </div>

          <div className="bg-white border border-gray-100 rounded-xl p-5 text-center shadow-sm hover:shadow-md transition">
            <h3 className="text-3xl md:text-4xl font-extrabold text-blue-600">200+</h3>
            <p className="text-gray-500 text-sm mt-1">Active Campaigns Managed</p>
          </div>
        </div>
      </section>

      {/* CTA - Compact gradient with better proportions */}
      <section className="max-w-7xl mx-auto px-6 lg:px-12 py-12 lg:py-16">
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl p-8 md:p-10 text-center text-white shadow-lg">
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold mb-3">
            Ready to Scale with Google Ads?
          </h2>

          <p className="text-blue-100 text-base md:text-lg max-w-xl mx-auto mb-6">
            Let's build a campaign that drives qualified leads and grows your
            revenue – without wasting your ad budget.
          </p>

          <button className="bg-white text-blue-600 px-7 py-2.5 rounded-lg font-semibold hover:bg-gray-50 transition shadow-md hover:shadow-lg">
            Contact Us →
          </button>
        </div>
      </section>
    </div>
  );
};

export default GoogleAdsManagement;